...
class ApiCallServiceTest extends BaseServiceTest {
    ...
    void countOdsayApiCallSuccess() {
        ...
        fixtureGenerator.generateApiCall(ClientType.GOOGLE_MAPS, 3, now);
    ...
    @DisplayName("이번달 GoogleMaps API 호출 횟수를 조회한다.")
    @Test
    void countGoogleMapsApiCallSuccess() {
        LocalDate now = LocalDate.now();
        LocalDate lastMonth = now.minusMonths(1);

        fixtureGenerator.generateApiCall(ClientType.GOOGLE_MAPS, 1, lastMonth);
        fixtureGenerator.generateApiCall(ClientType.GOOGLE_MAPS, 3, now);
        fixtureGenerator.generateApiCall(ClientType.ODSAY, 2, now);

        ApiCallCountResponse actual = apiCallService.countGoogleMapsApiCall();
    ...
    @Test
    void countGoogleMapsApiCallFailure() {
        ApiCallCountResponse actual = apiCallService.countOdsayApiCall();
        ApiCallCountResponse expected = new ApiCallCountResponse(0);
