...
class FcmEventSchedulerTest extends BaseServiceTest {
    ...
    void testScheduledNotificationIsSentAtCorrectTime() throws InterruptedException {
        ...
        FcmGroupSendRequest fcmGroupSendRequest = new FcmGroupSendRequest(notification);
        ...
        doAnswer(invocation -> {
            latch.countDown();
            return null;
        }).when(fcmPushSender).sendPushNotification(fcmGroupSendRequest);

        // 2초후에 메세지가 가도록 설정한 fcmSendRequest를 인자로 넣어 실제 sendPushNotification()를 호출한다.
        fcmPushSender.sendPushNotification(fcmGroupSendRequest);

        // latch의 카운트가 0이될 때까지 대기할 시간을 정의한다.
        assertThat(latch.await(2, TimeUnit.SECONDS)).isTrue();

        // sendPushNotification() 메서드가 호출되었는지 검증한다.
        verify(fcmPushSender).sendPushNotification(fcmGroupSendRequest);
    }
}
