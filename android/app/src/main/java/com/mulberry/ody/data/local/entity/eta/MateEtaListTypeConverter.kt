...
class MateEtaListTypeConverter {
    ...
    private fun toJson(mateEta: MateEta): String {
        val etaStatusString = when (mateEta.etaStatus) {
    ...
    private fun fromJson(json: String): MateEta {
        ...
        val jsonObject = cleanedJson.split(",").map { it.trim() }
            .associate {
                val (key, value) = it.split(":", limit = 2).map { it.trim().removeSurrounding("\"") }
                key to value
            }
        ...
        val etaStatusMap = etaStatusJson.removePrefix("{").removeSuffix("}").split("/")
            .map { it.trim() }
            .associate {
                val (key, value) = it.split(":", limit = 2).map { it.trim() }
                key.removeSurrounding("\"") to value.trim().removeSurrounding("\"") // " 제거
            }

        val etaStatusType = etaStatusMap["type"] ?: throw IllegalArgumentException("Missing etaStatus type")
        val etaStatus = when (etaStatusType) {
            "Arrived" -> EtaStatus.Arrived
            "ArrivalSoon" -> {
                val durationMinutes = etaStatusMap["durationMinutes"]?.toInt() ?: throw IllegalArgumentException("Missing durationMinutes")
                EtaStatus.ArrivalSoon(durationMinutes)
            }
            "LateWarning" -> {
                val durationMinutes = etaStatusMap["durationMinutes"]?.toInt() ?: throw IllegalArgumentException("Missing durationMinutes")
                EtaStatus.LateWarning(durationMinutes)
            }
            "Late" -> {
                val durationMinutes = etaStatusMap["durationMinutes"]?.toInt() ?: throw IllegalArgumentException("Missing durationMinutes")
                EtaStatus.Late(durationMinutes)
            }
